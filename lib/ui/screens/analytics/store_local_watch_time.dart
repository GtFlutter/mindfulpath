...
class LocalAnalyticsStore {
  ...
  // static Future<AnalyticsResult> getOfflineWatchTime(DateTimeRange range) async {
  //   print(">>> getOfflineWatchTime CALLED with ==> ${range.start} - ${range.end}");
  //   final data = await getAllWatchTime();
  //   print(">>> Data loaded from SharedPreferences ==> ${data.isEmpty ? 'EMPTY' : 'FOUND'}");
  //
  //   final Map<String, int> dailyTotals = {};
  ...
  //       if (parsedDate == null) continue;
  //
  //       // Only include entries within the selected date range (not time)
  //       if (parsedDate.isBefore(startDate) || parsedDate.isAfter(endDate)) continue;
  //
  //       final catMap = entry.value as Map<String, dynamic>;
  //       for (final catEntry in catMap.entries) {
  //         final videoMap = catEntry.value as Map<String, dynamic>;
  //         for (final vidEntry in videoMap.entries) {
  //           final sec = (vidEntry.value as num).toInt();
  //           totalSeconds += sec;
  //           dailyTotals[dateStr] = (dailyTotals[dateStr] ?? 0) + sec;
  //         }
  //       }
  //     }
  //
  //     print("Offline analytics between~~~~~~ ${startDate.toIso8601String()} - ${endDate.toIso8601String()}");
  //     if (dailyTotals.isEmpty) {
  //       print("⚠️ No matching offline watch entries found in range.");
  //     } else {
  //       dailyTotals.forEach((date, sec) {
  //         print("  $date : $sec sec");
  //       });
  //     }
  //     print("Total offline watch time~~~~~~~~: $totalSeconds sec");
  //   } else {
  //     print("⚠️ No 'video' key found in offline data");
  //   }
  //
  //   // Convert totals to Statistic objects
  //   final statistics = dailyTotals.entries.map((e) {
